...
func NewS3(bucket string, flags *FlagStorage, config *S3Config) (*S3Backend, error) {
	...
	configfile := path.Join(os.Getenv("HOME"), "/.zscloud/config.ini")
	...
	hosts := strings.TrimSpace(cfg.Section("default").Key("endpoints").String())
	// logger.Info("endpoints : " + hosts)

	endpoints := strings.Split(hosts, ",")
	for _, v := range endpoints {
		logger.Info(v)
	}
	s.balancer, err = balancer.Build(balancer.LeastLoadBalancer, endpoints)
	if err != nil {
		return nil, err
	}
...
func (s *S3Backend) next() {
	s.S3.Client.ClientInfo.Endpoint = Next(s.balancer, s.ip)
	logger.Info("endpoint : " + s.S3.Client.ClientInfo.Endpoint)
}
